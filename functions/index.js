...
/**
 * Eliminar usuario completamente (solo para super admins)
 * Borra cuenta de Authentication, datos de Firestore y PDFs asociados
 */
exports.deleteUserCompletely = functions.https.onCall(async (data, context) => {
  ...
  console.log(`🔍 Intentando eliminar usuario ${userId}, solicitado por ${callerId}`)

  try {
    // Verificar que el caller sea super admin
    const callerDoc = await admin.firestore().collection('users').doc(callerId).get()
    
    if (!callerDoc.exists) {
      console.error(`❌ Caller ${callerId} no existe en Firestore`)
      throw new functions.https.HttpsError('not-found', 'Caller no encontrado')
    }

    const callerData = callerDoc.data()
    const callerEmail = context.auth.token.email || ''
    const isSuperAdmin = callerEmail === 'franco_burgoa1@hotmail.com'
    const isAdmin = callerData?.role === 'admin'

    console.log(`👤 Caller: ${callerEmail}, Super Admin: ${isSuperAdmin}, Admin: ${isAdmin}`)

    if (!isSuperAdmin && !isAdmin) {
      throw new functions.https.HttpsError('permission-denied', 'Solo super admins y admins pueden eliminar usuarios')
    }

    // No permitir que se elimine a sí mismo
    if (userId === callerId) {
      throw new functions.https.HttpsError('invalid-argument', 'No puedes eliminar tu propia cuenta desde el panel de admin')
    }
    ...
    if (!userDoc.exists) {
      console.log(`⚠️ Usuario ${userId} no existe en Firestore, continuando con Auth...`)
    }
    ...
    // 4. Borrar cuenta de Firebase Authentication
    try {
      await admin.auth().deleteUser(userId)
      console.log(`✅ Cuenta de Authentication eliminada`)
    } catch (authError) {
      // Si la cuenta de Auth no existe o ya fue borrada, continuar
      console.log(`⚠️ No se pudo borrar cuenta de Auth: ${authError.message}`)
      // No lanzar error, continuar
    }

    console.log(`🎉 Usuario ${userEmail} (${userId}) eliminado completamente`)
    ...
    // Sino, crear un nuevo HttpsError con el mensaje
  ...
  try {
    ...
    const callerDoc = await admin.firestore().collection('users').doc(callerId).get()
    const callerData = callerDoc.data()
    const isSuperAdmin = context.auth.token.email === 'franco_burgoa1@hotmail.com'
    const isAdmin = callerData?.role === 'admin'
    ...
    try {
      ...
      // Si la cuenta de Auth no existe o ya fue borrada, continuar
      console.log(`⚠️ No se pudo borrar cuenta de Auth (puede que ya esté eliminada): ${authError.message}`)
    }
